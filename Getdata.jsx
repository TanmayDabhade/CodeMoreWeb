...
const swift = [
  ...
  const welcome = [
    {
      "id":1,
      "name": "choose a language to start learning",
      "description" : ""
    }
  ]
...
const Getdata = ({prop}) => {
  ...
  const [file, setFile] = useState(python)
  ...
  function getFile(prop){
    if(prop === '/start-learning-python'){
      setTitle('Python')
      setFile(python)
    }
    else if(prop === '/start-learning-cpp'){
      setTitle('C++')
      setFile(cpp)
    }else if(prop==='/start-learning-swift'){
      setTitle('Swift')
      setFile(swift)
    }else{
    ...
    <div className='w-full'>
      ...
      <div className="w-full">
        <div className="flex flex-wrap justify-center gap-3 border-2 border-solid border-black bg-white px-3 py-2 rounded-xl">
          <button 
            onClick={() => {setFile(python); setSelectedId(null); setTitle("Python")}}
            className='outline-none px-4 rounded-xl'
          >
            Python
          </button>
          <button 
            onClick={() => {setFile(cpp); setSelectedId(null); setTitle("C++")}}
            className='outline-none px-4 rounded-xl'
          >
            C++
          </button>
          <button 
            onClick={() => {setFile(swift); setSelectedId(null);setTitle("Swift")}}
            className='outline-none px-4 rounded-xl'
          >
            Swift
          </button>
